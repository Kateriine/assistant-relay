...
router.post('/processOAuth', async (req, res) => {
  try {
    ...
    await sendTextInput(
      `broadcast Assistant Relay is setup and running for ${name}`,
      name
    );
    res.status(200).send();
  } catch (e) {
...
router.get('/audio', async (req, res) => {
  res.sendFile(
    path.resolve(__dirname, `../bin/audio-responses/${req.query.v}.wav`)
  );
});

router.get('/html', async (req, res) => {
  res.sendFile(
    path.resolve(__dirname, `../bin/html-responses/${req.query.v}.html`)
  );
});
